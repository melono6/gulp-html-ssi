	...
	function processInserts(file) {
		...
		while(fndx > -1) {
			...
			if(insertFiles[filename]) {
                ...
                var arrayreg = new RegExp(/{{\#(.*?)}}([\s\S]*?){{\/(.*?)}}/, 'g');
                var standardreg = new RegExp(/{{(?!\?)(?!\/)(.*?)}}/, 'g');
                
                while ((match = arrayreg.exec(insertFiles[filename].content)) !== null) {
                    var array,
                        newinc = '';
                    if (attrs[match[1]]) {
                        
                        try {
                            attrs[match[1]] = attrs[match[1]].replace(new RegExp("'", "g"), "\"");
                            array = JSON.parse(attrs[match[1]]);
                        } catch (e) {
                            array = attrs[match[1]].split(',');
                        }
                        array.forEach((value) => {
                            if (typeof value == 'object') {
                                let thisinc = match[2];
                                Object.keys(value).forEach((key) => {
                                    thisinc = thisinc.replace(new RegExp('{{' + key + '}}', 'g'), value[key]);
                                });
                                newinc += thisinc;
                            } else {
                                newinc += match[2].replace(new RegExp("{{value}}", "g"), value);
                            }
                            
                        });
                        inc = inc.replace(match[0], newinc);
                    } else {
                        inc = inc.replace(match[0], '');
                    }
                }
                
                while ((match = truereg.exec(insertFiles[filename].content)) !== null) {
                    var index,
                        iftext,
                        elsetext;
                ...
                while ((match = standardreg.exec(insertFiles[filename].content)) !== null) {
                    if (attrs[match[1]]) {
                        inc = inc.replace(new RegExp('{{' + match[1] + '}}', 'g'), attrs[match[1]]);
                    } else {
							  inc = inc.replace(new RegExp('{{' + match[1] + '}}', 'g'), '');
						  }
                }
